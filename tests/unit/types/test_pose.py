...
def test_pose_from_dict():
    ...
    pose_dict = {"position": {"x": 1.0, "y": 2.0, "z": 3.0}, "orientation": {"x": 0.1, "y": 0.2, "z": 0.3}}
    pose = Pose(**pose_dict)
    assert isinstance(pose, Pose), "Pose should be a Pose object"
    assert isinstance(pose.position, Vector3d), "Position should be a Vector3d object"
    assert isinstance(pose.orientation, Vector3d), "Orientation should be a Vector3d object"
...
def test_pose_multiplication(pose1, pose2):
    ...
    assert len([result_position.x, result_position.y, result_position.z]) == 3, "Position should have 3 coordinates"

    # Test orientation
    result_orientation = result_pose.orientation
    assert isinstance(result_orientation, Vector3d), "Resulting orientation should be a Vector3d object"
    assert (
...
def test_matrix_multiplication(pose1, identity_matrix):
    ...
    assert isinstance(result_pose, Pose), "Multiplying a Pose with a matrix should return a Pose object"
    assert np.allclose(result_pose.matrix, pose1.matrix), "Multiplying with identity matrix should return the same pose"
...
def test_reverse_matrix_multiplication(pose1, identity_matrix):
    ...
    assert np.allclose(result_pose.matrix, pose1.matrix), "Multiplying by identity matrix should not change the pose"
...
def test_matrix_multiplication_against_np_dot(pose1, pose2, combined_matrix):
    ...
    assert np.allclose(result_pose.matrix, result_matrix), "Matrix multiplication should be consistent with np.dot"
...
def test_rotation_matrix_multiplication():
    ...
    assert np.allclose(result_pose.position.as_array(), pose.position.as_array()), "Position should remain unchanged"
