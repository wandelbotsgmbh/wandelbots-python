...
def get_current_joint_config(instance: Instance, cell: str, motion_group: str) -> list[float]:
    ...
    logger.debug(f"Getting current joint config for cell {cell} and motion group {motion_group} on: {url}")
    code, response = get(url, instance=instance)
    if code != 200:
        logger.error(f"Failed to get current joint config for cell {cell} and motion group {motion_group}")
        return []
...
def get_current_tcp_pose(instance: Instance, cell: str, motion_group: str, tcp: str) -> Optional[Pose]:
    ...
    logger.debug(f"Getting current tcp pose for cell {cell} and motion group {motion_group} and tcp {tcp} on: {url}")
    code, response = get(url, instance=instance)
    if code != 200:
        logger.error(f"Failed to get current tcp pose for cell {cell} and motion group {motion_group}")
        return None
    motion_group_state: MotionGroupState = MotionGroupStateResponse.from_dict(response).state
