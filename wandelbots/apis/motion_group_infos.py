...
def get_tcps(instance: Instance, cell: str, motion_group: str) -> list[str]:
    ...
    logger.debug(
        f"Getting tcps for cell {cell} and motion group {motion_group} on: {url}"
    )
    code, response = get(url, instance=instance)
    if code != 200:
        logger.error(
            f"Failed to get tcps for cell {cell} and motion group {motion_group}"
        )
        return []
...
def get_current_joint_config(
    ...
    if code != 200:
        ...
        return []
    motion_group_state: MotionGroupState = MotionGroupStateResponse.from_dict(
        response
    ).state
